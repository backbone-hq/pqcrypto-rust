//! Portable ("soft") NTT backend for ML-DSA (FIPS 204).
//! Pure Rust implementation using Montgomery arithmetic with i32 coefficients.
//!
//! Always compiled — acts as the fallback on x86_64 when AVX2 is not available
//! at compile time, and as the sole backend on non-x86 targets.
//!
...
use crate::field::montgomery_reduce;
use crate::poly::Poly;

// Precomputed zeta values
pub(super) const ZETAS: [i32; 256] = [
    ...
    while len >= 1 {
        for start in (0..256).step_by(2 * len) {
            ...
            for j in start..start + len {
                let t = montgomery_reduce(i64::from(p.coeffs[j + len]) * i64::from(zeta));
...
pub(crate) fn inv_ntt(p: &mut Poly) {
    ...
    while len <= 128 {
        for start in (0..256).step_by(2 * len) {
            ...
            for j in start..start + len {
                ...
                p.coeffs[j] = a.wrapping_add(b);
                p.coeffs[j + len] = montgomery_reduce(i64::from(a.wrapping_sub(b)) * zeta);
            }
        }
        len *= 2;
    }
    const F: i32 = 41978;
    for i in 0..256 {
        p.coeffs[i] = montgomery_reduce(i64::from(p.coeffs[i]) * i64::from(F));
    }
}

pub(crate) fn ntt_mul(a: &Poly, b: &Poly, c: &mut Poly) {
    for i in 0..256 {
        c.coeffs[i] = montgomery_reduce(i64::from(a.coeffs[i]) * i64::from(b.coeffs[i]));
    }
}
...
mod tests {
    ...
    fn test_ntt_roundtrip() {
        let cases: &[(usize, i32, i32)] = &[(0, 1, -4186625), (0, 1234, -3958378)];
        for &(pos, val, expected_val) in cases {
            let mut p = Poly::new();
            p.coeffs[pos] = val;
            ...
            for i in 0..256 {
                if i == pos {
                    assert_eq!(
                        p.coeffs[i], expected_val,
                        "NTT roundtrip (pos {}, val {}): coeff[{}] got {}, expected {}",
                        pos, val, i, p.coeffs[i], expected_val
                    );
                } else {
                    assert_eq!(
                        p.coeffs[i], 0,
                        "NTT roundtrip (pos {}, val {}): coeff[{}] non-zero: {}",
                        pos, val, i, p.coeffs[i]
                    );
                }
            }
        }
    ...
    fn test_ntt_unit_vector() {
        ...
        for i in 0..256 {
            assert_eq!(
                p.coeffs[i], 1234,
                "NTT of unit vector at pos 0: coeff[{}] = {}, expected 1234",
                i, p.coeffs[i]
            );
        }
    }
