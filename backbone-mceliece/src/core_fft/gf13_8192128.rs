...
pub(crate) const SYS_T: usize = 128;
pub(crate) const SYS_N: usize = 8192;
pub(crate) const PK_NROWS: usize = 1664;
pub(crate) const PK_NCOLS: usize = 6528;
pub(crate) const PK_ROW_BYTES: usize = 816;
pub(crate) const SYND_BYTES: usize = 208;
pub(crate) const IRR_BYTES: usize = 256;
pub(crate) const COND_BYTES: usize = 12800;
pub(crate) const GFMASK: u16 = 8191u16;
pub(crate) const CRYPTO_PUBLICKEYBYTES: usize = 1357824;
pub(crate) const CRYPTO_SECRETKEYBYTES: usize = 14120;
pub(crate) const CRYPTO_CIPHERTEXTBYTES: usize = 208;
pub(crate) const CRYPTO_BYTES: usize = 32;

/// Post-process step for the forward FFT (GFBITS=13, 8192128 variant).
/// XORs the FFT output with the pre-computed powers.
fn fft_postprocess(out: &mut [[Vec64; GFBITS]; 128], powers: &[[u64; 13]; 128]) {
    for i in 0..128 {
        for b in 0..GFBITS {
            out[i][b] ^= powers[i][b];
        }
...
/// Forward Gao-Mateer FFT (128-point, GFBITS=13, 8192128 variant).
/// Post-processing XORs with `powers` (variant-specific).
pub(crate) fn fft(
    out: &mut [[Vec64; GFBITS]; 128],
    inp: &mut [[Vec64; GFBITS]; 2],
    consts: &[[u64; 13]; 128],
...
pub(crate) fn support_gen(support: &mut [Gf; SYS_N], c: &[u8]) {
    ...
    for i in 0..(1 << GFBITS) {
        let a = bitrev(u16::try_from(i).expect("i < 8192 fits in u16"));
        for j in 0..GFBITS {
    ...
    for i in 0..SYS_N {
        ...
        for j in (0..GFBITS).rev() {
            support[i] <<= 1;
            support[i] |=
                u16::try_from((l_full[j][i >> 6] >> (i & 63)) & 1).expect("bit value fits in u16");
        }
    }
}

/// Reverse the lower 13 bits of `value` (bit-reversal permutation).
...
fn gf_sq(inp: Gf) -> Gf {
    ...
    t = x & 0x007E000;
    x ^= (t >> 9) ^ (t >> 10) ^ (t >> 12) ^ (t >> 13);
    u16::try_from(x & u32::from(GFMASK)).expect("result fits in u16")
}

/// Square a GF(2^13) element twice (i.e. compute x^(2^2)).
fn gf_sq2(inp: Gf) -> Gf {
    const B: [u64; 4] = [
    ...
    u16::try_from(x & u64::from(GFMASK)).expect("result fits in u16")
}

/// Compute `sq(inp) * m` in GF(2^13).
fn gf_sqmul(inp: Gf, m: Gf) -> Gf {
    gf_mul(gf_sq(inp), m)
}

/// Compute `sq2(inp) * m` in GF(2^13).
...
/// Generate the Goppa polynomial generator matrix row.
...
/// Convert a flat `u64` array into two bitsliced 128-element arrays.
fn to_bitslicing_2x(
...
/// Generate the McEliece public key from the secret key components.
pub(crate) fn pk_gen(
    pk: &mut Vec<u8>,
    irr: &[u8],
    perm: &[u32],
    ...
    for i in 0..(1 << GFBITS) {
        pi_out[i] = i16::try_from(list[i] & u64::from(GFMASK)).expect("value fits in i16");
    }
...
fn cbrecursion_write(out: &mut [u8], pos: usize, step: usize, pi: &[i16], w: usize, n: usize) {
    if w == 1 {
        out[pos >> 3] ^= u8::try_from(pi[0] & 0xff).expect("lsb of i16 fits in u8") << (pos & 7);
        return;
    }
