//! Higher-confidence validation on the NTRU LPRime public API.
//!
...
use backbone_ntruplr::ntruplr761;
...
fn decaps_never_panics_on_random_ciphertexts() {
    let (pk, sk) = ntruplr761::keygen_with_rng(&mut FixedRng::new(vec![3u8; 48])).expect("keygen");
    let enc = ntruplr761::encaps_with_rng(&pk, &mut FixedRng::new(vec![7u8; 48])).expect("encaps");
    let ct_len = enc.ciphertext.len();
    let mut rng = XorShift::new();
    for _ in 0..1000 {
        let mut ct = vec![0u8; ct_len];
        rng.fill(&mut ct);
        let _ = ntruplr761::decaps(&sk, &ct);
    }
    for _ in 0..300 {
...
fn encaps_never_panics_on_garbage_public_keys() {
    let (pk, _sk) = ntruplr761::keygen_with_rng(&mut FixedRng::new(vec![3u8; 48])).expect("keygen");
    let pk_len = pk.pk.len();
    let mut rng = XorShift::new();
    for _ in 0..300 {
        let mut pk_bytes = vec![0u8; pk_len];
        rng.fill(&mut pk_bytes);
        if let Ok(garbage) = ntruplr761::PublicKey::from_bytes(&pk_bytes) {
            let _ = ntruplr761::encaps(&garbage);
        }
    }
}
