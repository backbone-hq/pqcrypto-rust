...
use alloc::vec;
use core::fmt;
use pqcrypto_utils::ct::ct_count_nonzero;
use pqcrypto_utils::karatsuba::{karatsuba_mul, reduce_ring};
...
impl<const P: usize, const Q: i16> Rq<P, Q> {
    ...
    pub(crate) fn encode(&self, out: &mut [u8]) -> Result<(), &'static str> {
        ...
        let values: vec::Vec<u64> = (0..P)
            ...
            .collect();
        let enc = pqcrypto_utils::tree_encode::rounded_encode(values, alloc::vec![q; P]);
    ...
    pub(crate) fn decode(input: &[u8]) -> Result<Self, &'static str> {
        ...
        let values = pqcrypto_utils::tree_encode::rounded_decode(&input[..encoded_bytes], q, P)
    ...
    pub(crate) fn encode_rounded(&self) -> vec::Vec<u8> {
        ...
        pqcrypto_utils::tree_encode::rounded_encode(values, alloc::vec![m_val; P])
    }
    ...
    pub(crate) fn decode_rounded(input: &[u8]) -> Result<Self, &'static str> {
        ...
        let values = pqcrypto_utils::tree_encode::rounded_decode(input, m_val, P)
