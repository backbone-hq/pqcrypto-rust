...
macro_rules! define_variant {
    ($params:ident, $ss_size:expr, $doc_variant:expr, $doc_sec:expr) => {
        ...
        pub fn keygen(seed: &[u8]) -> Result<(PublicKey, SecretKey), Error> {
            ...
            $crate::kem::keypair::<_P, _Q>(&mut pk, &mut sk, seed, _W)?;
        ...
        mod tests {
            ...
            fn test_keygen_different_seeds() {
                let (pk1, _) = keygen(b"seed123456789012345678901234567890").unwrap();
                let (pk2, _) = keygen(b"SEED123456789012345678901234567890").unwrap();
                assert_ne!(pk1.pk, pk2.pk);
            }

            #[test]
            fn test_keygen_roundtrip() {
                let seed = b"test_seed_1234567890123456789012345";
            ...
            fn test_encaps_deterministic() {
                let seed = b"deterministic_keygen_test_1234567";
                let r_seed = b"same_r_seed_1234567890123456789012";
                let (pk, _sk) = keygen(seed).unwrap();
                let enc1 = encaps_deterministic(&pk, r_seed).unwrap();
                let enc2 = encaps_deterministic(&pk, r_seed).unwrap();
            ...
            fn test_negative_wrong_key() {
                ...
                let (_, sk_b) = keygen(b"aaaaaaaabbbbbbbbccccccccdddddddd").unwrap();
            ...
            fn test_negative_corrupted_ct() {
                ...
                let result = decaps(&sk, &enc.ciphertext).unwrap();
                assert_ne!(enc.shared_secret, result);
            }
        }
    };
