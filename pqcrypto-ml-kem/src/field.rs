...
/// Matches pq-crystals/kyber reference C code behavior exactly.
#[inline]
pub(crate) fn montgomery_reduce(a: i32) -> i16 {
    // t = (int16_t)a * QINV  (C: result computed as int32_t, stored as int16_t)
    let a_bytes = a.to_ne_bytes();
    let a_i16 = i16::from_ne_bytes([a_bytes[0], a_bytes[1]]);
    // t = a_i16 * QINV (wrapping i16 multiplication promoted to i32, then truncated)
    ...
    // r = (a - (int32_t)t * KYBER_Q) >> 16
    let r = a.wrapping_sub(i32::from(t).wrapping_mul(Q)) >> 16;
    // Safe: r is guaranteed to be in [-Q+1, Q-1] which fits in i16
...
pub(crate) fn barrett_reduce(a: i16) -> i16 {
    const V: i32 = 20159; // ((1<<26) + Q/2) / Q
                          // t = ((V * a + 1<<25) >> 26) is small (≈ a/Q), fits in i16
...
mod tests {
    ...
    fn test_montgomery_reduce_r() {
        // For a = x * R, montgomery_reduce(a) = x
    ...
    fn test_mul_consistent() {
        // mul(a,b) = montgomery_reduce(a*b) = a*b*R^{-1} mod Q
        // Verify consistency: mul(a,b) then multiply by another gives same result path
        let a: i16 = 1234i16;
        let b: i16 = 567i16;
        let c: i16 = 89i16;
        // (a*b*R^{-1}) * c * R^{-1} = a*b*c * R^{-2} mod Q
        let ab = montgomery_reduce(i32::from(a) * i32::from(b));
        let abc = montgomery_reduce(i32::from(ab) * i32::from(c));
        // Compare with (b*c) first, then a
    ...
    fn test_montgomery_self_consistent() {
        // Verify that montgomery_reduce on a value and then on result
        // wrapped in multiplication chain gives consistent results
        let x: i16 = 1234;
        // Convert to "Montgomery form" by multiplying by R^2 and reducing
        // montgomery_reduce(x * R^2) = x * R^2 * R^{-1} = x * R mod Q
        // This is standard Montgomery form
        let mont_x = montgomery_reduce(i32::from(x).wrapping_mul(1353)); // 1353 = R^2 mod Q
                                                                         // Convert back: montgomery_reduce(mont_x) = (x * R) * R^{-1} = x
